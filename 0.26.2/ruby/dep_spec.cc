...
namespace
{
    ...
    {
        tr1::shared_ptr<const WrappedSpecBase> wrapped;
        VALUE klass;

        void visit_leaf(const PackageDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<PackageDepSpec>(tr1::static_pointer_cast<PackageDepSpec>(spec.clone())));
            klass = c_package_dep_spec;
        }

        void visit_leaf(const BlockDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<BlockDepSpec>(tr1::static_pointer_cast<BlockDepSpec>(spec.clone())));
            klass = c_block_dep_spec;
        }

        void visit_leaf(const PlainTextDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<PlainTextDepSpec>(tr1::static_pointer_cast<PlainTextDepSpec>(spec.clone())));
            klass = c_plain_text_dep_spec;
        }

        void visit_leaf(const SimpleURIDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<SimpleURIDepSpec>(tr1::static_pointer_cast<SimpleURIDepSpec>(spec.clone())));
            klass = c_simple_uri_dep_spec;
        }

        void visit_leaf(const FetchableURIDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<FetchableURIDepSpec>(tr1::static_pointer_cast<FetchableURIDepSpec>(spec.clone())));
            klass = c_fetchable_uri_dep_spec;
        }

        void visit_leaf(const URILabelsDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<URILabelsDepSpec>(tr1::static_pointer_cast<URILabelsDepSpec>(spec.clone())));
            klass = c_uri_labels_dep_spec;
        }

        void visit_leaf(const DependencyLabelsDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<DependencyLabelsDepSpec>(tr1::static_pointer_cast<DependencyLabelsDepSpec>(spec.clone())));
            klass = c_dependency_labels_dep_spec;
        }

        void visit_leaf(const NamedSetDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<NamedSetDepSpec>(tr1::static_pointer_cast<NamedSetDepSpec>(spec.clone())));
            klass = c_named_set_dep_spec;
        }

        void visit_leaf(const LicenseDepSpec & spec)
        {
            wrapped.reset(new WrappedSpec<LicenseDepSpec>(tr1::static_pointer_cast<LicenseDepSpec>(spec.clone())));
        ...
        {
            wrapped.reset((new WrappedSpec<ConditionalDepSpec>(tr1::static_pointer_cast<ConditionalDepSpec>(spec.clone())))->add_children(cur, end));
        ...
        void visit_sequence(const AllDepSpec & spec,
                ...
                GenericSpecTree::ConstSequenceIterator end)
        {
            wrapped.reset((new WrappedSpec<AllDepSpec>(tr1::static_pointer_cast<AllDepSpec>(spec.clone())))->add_children(cur, end));
        ...
        void visit_sequence(const AnyDepSpec & spec,
                ...
                GenericSpecTree::ConstSequenceIterator end)
        {
            wrapped.reset((new WrappedSpec<AnyDepSpec>(tr1::static_pointer_cast<AnyDepSpec>(spec.clone())))->add_children(cur, end));
        ...
        each(VALUE self)
        {
            ...
            if ((*ptr)->children())
                for (WrappedSpecBase::Children::const_iterator i((*ptr)->children()->begin()), i_end((*ptr)->children()->end()) ;
                        i != i_end ; ++i)
                {
                    tr1::shared_ptr<const WrappedSpecBase> * newptr(new tr1::shared_ptr<const WrappedSpecBase>(i->second));
                    rb_yield(Data_Wrap_Struct(i->first, 0, &Common<tr1::shared_ptr<const WrappedSpecBase> >::free, newptr));
                }
            return self;
        }
    };
...
paludis::ruby::dep_tree_to_value(const tr1::shared_ptr<const typename T_::ConstItem> & m)
{
    try
    {
        TreeToValue v;
        m->accept(v);
        tr1::shared_ptr<const WrappedSpecBase> * ptr(new tr1::shared_ptr<const WrappedSpecBase>(v.wrapped));
        return Data_Wrap_Struct(v.klass, 0, &Common<tr1::shared_ptr<const WrappedSpecBase> >::free, ptr);
    }
    catch (const std::exception & e)
    {
        exception_to_ruby_exception(e);
